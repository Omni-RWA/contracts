...
contract HookScript is Script {
    ...
    function run() public {
        ...
        (address hookAddress, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, flags, type(Hook).creationCode, abi.encode(address(POOLMANAGER), APP_ID, ACTION_ID)
        );

        // Deploy the hook using CREATE2
        vm.broadcast();
        Hook rwa = new Hook{salt: salt}(IPoolManager(address(POOLMANAGER)), APP_ID, ACTION_ID);
