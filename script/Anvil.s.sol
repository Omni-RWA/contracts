...
contract HookScript is Script {
    ...
    function run() public {
        ...
        (address hookAddress, bytes32 salt) =
            HookMiner.find(CREATE2_DEPLOYER, permissions, type(Hook).creationCode, abi.encode(address(manager)));

        // ----------------------------- //
        // Deploy the hook using CREATE2 //
        // ----------------------------- //
        vm.broadcast();
        Hook rwa = new Hook{salt: salt}(manager, APP_ID, ACTION_ID);
        require(address(rwa) == hookAddress, "HookScript: hook address mismatch");
        ...
        vm.startBroadcast();
        testLifecycle(manager, address(rwa), lpRouter, swapRouter);
        vm.stopBroadcast();
    }
