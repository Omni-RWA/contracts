...
contract CreatePool is Ownable {
    ...
    // deploy the hook if hook is empty address
    function deployHookIfNeeded() public {
        if (hook == Hook(address(0))) {
            deployHook();
        }
    }
    ...
    function createPoolAndProvideLiquidity(
        ...
        deployHookIfNeeded();
        (key, id) = initPool(_currency0, _currency1, hook, _fee, _sqrtPriceX96, _initData);
    ...
    function deployHook() public {
        ...
        (address hookAddress, bytes32 salt) =
            HookMiner.find(address(this), flags, type(Hook).creationCode, abi.encode(address(manager), appId, actionId));

        hook = new Hook{salt: salt}(IPoolManager(address(manager)), appId, actionId);
