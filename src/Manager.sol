...
import {Hook} from "./hooks/Hook.sol";
...
contract Manager is Ownable {
    ...
    {
        // create rwa
        rwa = new RWA({_name: params.name, _symbol: params.symbol, _initialOwner: address(this)});

        // mint rwa
        uint256 tokenId = rwa.mint({to: address(this), uri: params.uri, assetDetails: params.assetDetails});

        // approve tokenId to the vault factory
        rwa.approve(address(vaultFactory), tokenId);

        // create vault (erc20 token)
        Vault vault = Vault(
            vaultFactory.createVault({
                _collection: address(rwa),
                _tokenId: tokenId,
        ...
        (address hookAddress, bytes32 salt) = HookMiner.find(
            address(this), flags, type(Hook).creationCode, abi.encode(address(manager), APP_ID, ACTION_ID)
        );

        Hook hook = new Hook{salt: salt}(IPoolManager(address(manager)), APP_ID, ACTION_ID);
        ...
        return key;
        // return (rwa, vault, createPool, key, id, liquidityResult);
    }
}
